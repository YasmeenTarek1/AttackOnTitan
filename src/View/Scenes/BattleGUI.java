...
import Controller.CharactersSceneController;
import Controller.MainController;
import Controller.ModeSceneController;
import Controller.StartMenuSceneController;
...
import View.GameElements.LanePane;
import View.GameElements.TitanPane;
import View.GameElements.WeaponPane;
...
public class BattleGUI extends Scene {
    ...
    private final int numOfLanes = ModeSceneController.getNumOfLanes();
    private final String ChosenMode = ModeSceneController.getMode();
    private final String ChosenCharacter = CharactersSceneController.getCharacter();
    private int chosenLaneToDeployWeapon;
    private Label score, resourcesValue, Turn, battlePhase;
    private HashMap<Titan, TitanPane> mappingTitans;
    private final HashMap<Lane, LanePane> mappingLanes;
    private final ArrayList<WeaponPane> Weapons;
    ...
    public BattleGUI(StackPane p) {
        ...
        MainController.playMusic("Media/Sounds/Battle Track.mp3");
        ...
        MainController.setBattleScene(this);

        gameLoop = new Timeline(new KeyFrame(Duration.seconds(0.1), event -> {
            if (!b.isGameOver()) {
                weaponShopIcon.getButton().setOnAction(e -> chooseLanesStage());
    ...
    private void CharacterAppearanceEarly() {
        ...
        String path;
        ImageView imageView, characterIntro, generalInfo;

        for (Node node : root.getChildren())
            node.setEffect(new GaussianBlur(15));

        generalInfo = new ImageView(new Image("file:Media/Visuals/At the start of each turn.png"));
        ...
        next.getButton().setOnAction(
                e -> {
                    ...
                    if (ChosenCharacter.equals("Mikasa"))
                        MainController.playMusic("Media/Sounds/At the start of each turn Women.mp3");
                    else
                        MainController.playMusic("Media/Sounds/At the start of each turn Man.mp3");
    ...
    public void DisplayEndScene() {
        ...
        MainController.playMusic("Media/Sounds/Game Over.mp3");
        ...
        Exit.getButton().setOnAction(e -> MainController.closeWindow());
    }
    ...
    private void chooseLanesStage() {
        ...
        ButtonGame lane1, lane2, lane3, lane4 = null, lane5 = null;
        ...
        chooseLanesStage.initOwner(MainController.getWindow());
        chooseLanesStage.show();

        lane1.getButton().setOnAction(e -> {
            chosenLaneToDeployWeapon = numOfLanes - 1;
            chooseLanesStage.close();
            weaponShopStage();
        });
        lane2.getButton().setOnAction(e -> {
            chosenLaneToDeployWeapon = numOfLanes - 2;
            chooseLanesStage.close();
            weaponShopStage();
        });
        lane3.getButton().setOnAction(e -> {
            chosenLaneToDeployWeapon = numOfLanes - 3;
            chooseLanesStage.close();
            weaponShopStage();
        });
        if (numOfLanes == 5) {
            lane4.getButton().setOnAction(e -> {
                chosenLaneToDeployWeapon = numOfLanes - 4;
                chooseLanesStage.close();
                weaponShopStage();
            });
            lane5.getButton().setOnAction(e -> {
                chosenLaneToDeployWeapon = numOfLanes - 5;
                chooseLanesStage.close();
                weaponShopStage();
            });
        }
    ...
    public void weaponShopStage() {
        ...
        ImageView goldButton = new ImageView(new Image("file:Media/Visuals/Gold Button.png"));
        goldButton.setFitWidth(350);
        goldButton.setFitHeight(200);
        ...
        StackPane money = new StackPane(goldButton, label);
        ...
        ButtonGame weaponButton1 = new ButtonGame(weapon1, 360, 170);// piercing
        ButtonGame weaponButton2 = new ButtonGame(weapon2, 300, 100);// volley
        ButtonGame weaponButton3 = new ButtonGame(weapon3, 350, 230); // sniper
        ButtonGame weaponButton4 = new ButtonGame(weapon4, 400, 200);// wall Trap

        weaponButton1.setLayoutX(240);
        weaponButton1.setLayoutY(185);

        weaponButton2.setLayoutX(600);
        weaponButton2.setLayoutY(210);

        weaponButton3.setLayoutX(750);
        weaponButton3.setLayoutY(600);

        weaponButton4.setLayoutX(100);
        weaponButton4.setLayoutY(630);
        ...
        Pane WeaponButtons = new Pane();
        WeaponButtons.getChildren().addAll(weaponButton1, weaponButton2, weaponButton3, weaponButton4);

        Pane ButtonsInfo = new Pane();
        ButtonsInfo.getChildren().addAll(PiercingInfo, SniperInfo, VolleyInfo, WallTrapInfo);

        weaponShopRoot.getChildren().addAll(ButtonsInfo, WeaponButtons);
        ...
        weaponButton1.setOnMouseEntered(event -> PiercingInfo.setVisible(true));
        weaponButton1.setOnMouseExited(event -> PiercingInfo.setVisible(false));
        weaponButton1.getButton().setOnAction(e -> {
            int weaponCode = 1;
            buy(weaponCode, chosenLaneToDeployWeapon);
            if (FinishWeaponShop)
                stage.close();
        });

        weaponButton2.setOnMouseEntered(event -> SniperInfo.setVisible(true));
        weaponButton2.setOnMouseExited(event -> SniperInfo.setVisible(false));
        weaponButton2.getButton().setOnAction(e -> {
            int weaponCode = 3;
            buy(weaponCode, chosenLaneToDeployWeapon);
            if (FinishWeaponShop)
                stage.close();
        ...
        weaponButton3.setOnMouseEntered(event -> VolleyInfo.setVisible(true));
        weaponButton3.setOnMouseExited(event -> VolleyInfo.setVisible(false));
        weaponButton3.getButton().setOnAction(e -> {
            int weaponCode = 2;
            buy(weaponCode, chosenLaneToDeployWeapon);
            if (FinishWeaponShop)
                stage.close();
        ...
        weaponButton4.setOnMouseEntered(event -> WallTrapInfo.setVisible(true));
        weaponButton4.setOnMouseExited(event -> WallTrapInfo.setVisible(false));
        weaponButton4.getButton().setOnAction(e -> {
            int weaponCode = 4;
            buy(weaponCode, chosenLaneToDeployWeapon);
            if (FinishWeaponShop)
                stage.close();
    ...
    private void buy(int code, int chosenLaneToDeployWeapon) {
        try {
            b.purchaseWeapon(code, b.getOriginalLanes().get(chosenLaneToDeployWeapon));
            deployWeapon(code, chosenLaneToDeployWeapon);
            MainController.switchToScene(MainController.getBattleScene());
            updateGUI();
    ...
    private void AI() {
        ...
        // Basic idea --> is to try all possibilities and choose the best
        // (Highest Score and if there is a tie, then Least amount of money spend)
        ...
        for (int i = 0; i < numOfLanes; i++) {     // loop on lanes
            ...
            for (int j = 1; j <= 4; j++) {         // loop on weapons
                ...
                Battle test = serialize();           // create new battle with the same state of b using Serialization
    ...
    private TitanPane convertTitanToTitanPane(Titan t) {
        ...
        if (t instanceof PureTitan) {
            ...
            titanPane = new TitanPane(new Image("file:Media/Visuals/Pure Titan.png"), width, height);
            ...
            titanPane = new TitanPane(new Image("file:Media/Visuals/Abnormal Titan.png"), width, height);
            ...
            titanPane = new TitanPane(new Image("file:Media/Visuals/Colossal Titan.png"), width, height);
            ...
            titanPane = new TitanPane(new Image("file:Media/Visuals/Armored Titan.png"), width, height);
        }
        return titanPane;
    }
